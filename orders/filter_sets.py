...
class OrderFilterSet(BootstrapFilterSet):
    ...
    customer = CharFilter(
        field_name="customer", method="find_by_full_name", widget=TextInput
    )
    is_open = BooleanFilter(
        field_name="is_open", label="Is open", widget=SelectBooleanWidget
    )
    car_type = MultipleChoiceFilter(
        choices=TypeChoices.choices, widget=CheckboxSelectMultiple
    )
    car_class = MultipleChoiceFilter(
        choices=ClassChoices.choices, widget=CheckboxSelectMultiple
    )
